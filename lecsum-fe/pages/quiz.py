...
def render_quiz_page():
    ...
    if not documents:
        st.info("요약된 문서가 없습니다. 먼저 문서를 업로드해주세요.")
        return

    doc_map = {
        f"{doc['name']} ({doc['created_at'][:10]})": doc["id"]
    ...
    if not st.session_state.selected_document_id:
        st.info("문서를 선택하고 '문서 확정'을 눌러주세요.")
    else:
        # --- 퀴즈 생성 섹션 ---
            ...
            if submit_button:
                if None in user_answers or "" in user_answers:
                    ...
                    with st.spinner("AI가 정답을 확인하고 피드백을 생성 중입니다..."):
                        try:
                            quiz_id_list = [q["id"] for q in quiz_data["quizzes"]]
