...
def get_enrichment_prompt() -> ChatPromptTemplate:
    ...
    - 특징: '강의 내용'과 '심화 학습'를 시각적으로 명확히 분리하여 제공,
    - [웹 검색 결과]가 없을 경우 AI가 조언
    """
    ...
    **🔎 심화 학습**
    ...
    return ChatPromptTemplate.from_messages([
        ...
        2. **구조화된 출력**:
           ...
           - **🔎 심화 학습**: 'Google 검색 결과'를 활용하여 최신 사례, 비유, 심화 정보를 덧붙여라.
             - **[웹 검색 결과]가 있는 경우**: 검색된 사례, 비유, 최신 정보를 활용해 설명하고 반드시 출처(URL)를 포함해라.
             - **[웹 검색 결과]가 없는 경우**: 너의 자체적인 지식을 활용하여 학생에게 도움이 될 만한 **암기 팁, 실무 사례, 혹은 주의해야 할 실수 패턴**을 조언해라.
        3. **URL 출처 표기**:
